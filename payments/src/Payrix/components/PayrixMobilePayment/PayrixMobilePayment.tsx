...
const PayrixMobilePayment: React.FC = () => {
  const [paymentInfo, setPaymentInfo] = useState<PayrixPaymentPageInfo | null>(
    null
  );
  ...
  const [paymentDataResponse, setPaymentDataResponse] =
    useState<PayrixPaymentPageInfo | null>(null);
  ...
  const checkIframeLoading = () => {
    ...
    const checkInterval = setInterval(() => {
      ...
      const iframesExist =
        ccnumberIframe && cvvIframe && ccexpIframe && googlePayIframe;

      if (iframesExist) {
        // Check if iframes have loaded content
        let allIframesReady = true;

        [ccnumberIframe, cvvIframe, ccexpIframe, googlePayIframe].forEach(
          (iframe: any) => {
            try {
        ...
        try {
          ...
          if (paymentResponse.success) {
            ...
            await handleLog(
              JSON.stringify({
                error:
                  "Mobile payment API failure after successful transaction",
    ...
    <>
      ...
      >
        <PayrixForm
          ...
          buttonText={`Pay $${
            paymentInfo ? (paymentInfo.amount / 100).toFixed(2) : "0.00"
          } USD`}
