...
const PaymentPageForm: React.FC<PaymentPageFormProps> = ({
    ...
    const notifyViaPostMessage = (status: string, data: any) => {
        try {
            ...
            if (status === "success") {
                setTimeout(() => {
                    window.location.href = `posapp://payment-completed?status=success&transactionId=${encodeURIComponent(data.transactionId || '')}&paymentIntentId=${encodeURIComponent(data.paymentIntentId || '')}`;
                }, 500);
            } else if (status === "failure") {
                setTimeout(() => {
                    window.location.href = `posapp://payment-completed?status=failure&error=${encodeURIComponent(data.error || 'An error occurred')}`;
                }, 500);
            }
            return true;
        } catch (e) {
            ...
            pr.on("paymentmethod", async (e) => {
                ...
                setTimeout(async () => {
                    try {
                        ...
                        if (!notified) {
                            // Fallback to URL scheme
                            window.location.href = `posapp://payment-completed?status=success&transactionId=${encodeURIComponent(paymentInfo.transactionId)}&paymentIntentId=${encodeURIComponent(paymentInfo.paymentIntentId)}`;
                        }
                        ...
                        if (!notified) {
                            // Fallback to URL scheme
                            window.location.href = `posapp://payment-completed?status=failure&error=${encodeURIComponent(processError.message || "Payment processing failed")}`;
                        }
                    }
                }, 1000); // 1 second delay
            });
    ...
    const shouldShowCancelButton = paymentInfo.metadata?.applicationName !== "TakeAway.App";
                        ...
                        <>
                            ...
                            {isCreditCard && surchargeFee > 0 && (
                                ...
                                >
                                    <span>ⓘ </span>
                                    <span>
                                        You will be assessed a surcharge amount
